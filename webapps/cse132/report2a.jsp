    ...
    <table border="1">
        <tr>
            ...
            <%
                    ...
                    rs2 = statement.executeQuery("SELECT CONFLICT.* FROM((SELECT COURSE_ID, COURSE_NAME, CONFLICT_COURSE_ID, CONFLICT_COURSE_NAME, COUNT(CONFLICT_COURSE_ID) AS GET_CONFLICT_COUNT FROM (SELECT DISTINCT c1.course_id AS COURSE_ID, c1.course_name AS COURSE_NAME, cl1.class_id AS CLASS_ID, c2.course_id AS CONFLICT_COURSE_ID, c2.course_name AS CONFLICT_COURSE_NAME, cl2.class_id AS CONFLICT_CLASS_ID FROM course c1, course c2, class cl1, class cl2, weekly_meetings w1, weekly_meetings w2, enroll_current e WHERE c1.course_id = cl1.course_id AND c2.course_id = cl2.course_id AND w1.class_id = cl1.class_id AND w2.class_id = cl2.class_id AND e.class_id = cl1.class_id AND w1.day_weekly = w2.day_weekly AND ((w1.start_weekly > w2.start_weekly AND w1.start_weekly < w2.end_weekly) OR (w1.end_weekly > w2.start_weekly AND w1.end_weekly < w2.end_weekly) OR (w1.start_weekly = w2.start_weekly AND w1.end_weekly = w2.end_weekly)) AND e.pid = '" + request.getParameter("ID") +"') AS temp GROUP BY COURSE_ID, COURSE_NAME, CONFLICT_COURSE_ID, CONFLICT_COURSE_NAME) AS TEMP1 INNER JOIN (SELECT c.course_id AS GET_COURSE_ID, COUNT(c.course_id) AS GET_COUNT FROM (SELECT DISTINCT e.class_id FROM enroll_current e) AS clt, class cl, course c WHERE clt.class_id = cl.class_id AND cl.course_id = c.course_id GROUP BY c.course_id) AS TEMP2 ON (TEMP1.GET_CONFLICT_COUNT = TEMP2.GET_COUNT AND TEMP1.CONFLICT_COURSE_ID = TEMP2.GET_COURSE_ID))AS CONFLICT WHERE CONFLICT.COURSE_ID != CONFLICT.CONFLICT_COURSE_ID");


                    /*
                    rs2 = statement.executeQuery("SELECT c.course_id AS GET_COURSE_ID, COUNT(c.course_id) AS GET_COUNT FROM (SELECT DISTINCT e.class_id FROM enroll_current e) AS clt, class cl, course c WHERE clt.class_id = cl.class_id AND cl.course_id = c.course_id GROUP BY c.course_id");
                    */
            ...
            %>
                    ...
                    <tr>
                        <form action="report2a.jsp" method="post">
                            ...
                            <%-- Get the ENROLLED_NUMBER --%>
                            <td>
                                <%= rs2.getString(1) %>
                            </td>


                            <%-- Get the ENROLLED_TITLE --%>
                            <td>
                                <%= rs2.getString(2) %>
                            </td>
